...
class TestRustCoverageGaps:
    # These cover Rust functions that instrumented coverage showed the rest of
    # the suite never reaches. Several are the Mutibs or MutableView twin of a
    # path that is already tested on Tibs or View.

    def test_tibs_f_property_matches_to_f(self):
        t = Tibs.from_f(1.5, 64)

        assert t.f == 1.5
        assert t.f == t.to_f()

    def test_mutable_view_view_keeps_settings_left_as_none(self):
        m = Mutibs("0x1234")

        assert repr(m.view().view(byte_order=ByteOrder.Little)) == (
            "MutableView(Mutibs('0x1234'), ByteOrder.Little, BitOrder.Msb0)"
        )
        assert repr(m.view().view(bit_order=BitOrder.Lsb0)) == (
            "MutableView(Mutibs('0x1234'), ByteOrder.Unspecified, BitOrder.Lsb0)"
        )
        assert repr(m.le.view()) == (
            "MutableView(Mutibs('0x1234'), ByteOrder.Little, BitOrder.Msb0)"
        )

    def test_mutable_view_be_and_msb0_undo_the_opposite_setting(self):
        m = Mutibs("0x1234")

        assert m.le.u == 13330
        assert m.le.be.u == 4660
        assert m.lsb0.bin == "0011010000010010"
        assert m.lsb0.msb0.bin == "0001001000110100"

    def test_mutable_view_octal_conversion(self):
        m = Mutibs("0xabc")

        assert m.view().to_oct() == "5274"
        assert m.view().oct == "5274"

    def test_mutable_view_to_tibs_materializes_copy(self):
        m = Mutibs("0x1234")

        t = m.view().to_tibs()
        m[0] = True

        assert t == Tibs("0x1234")
        assert m == Mutibs("0x9234")

    def test_mutable_view_octal_rejects_length_that_is_not_a_multiple_of_three(self):
        m = Mutibs("0x1234")
    ...
    def test_mutibs_reflected_operators(self):
        assert "0xf0" ^ Mutibs("0x0f") == Mutibs("0xff")
        assert 3 * Mutibs("0b10") == Mutibs("0b101010")
        assert "0x0f" + Mutibs("0xf0") == Mutibs("0x0ff0")

    def test_mutibs_extended_slice_matches_tibs(self):
        for key in (slice(None, None, 2), slice(None, None, -1), slice(1, 12, 3)):
            assert Mutibs("0x1234")[key] == Tibs("0x1234")[key]

        assert Mutibs("0x1234")[::2] == Mutibs("0x14")
        assert Mutibs("0x1234")[::-1] == Mutibs("0x2c48")

    def test_bool_iterator_is_its_own_iterator_and_reports_length_hint(self):
        it = iter(Tibs("0b1010"))
        ...
        assert list(it) == [False, True, False]
        assert operator.length_hint(it) == 0

    def test_mutibs_zstd_encode_round_trips(self):
        m = Mutibs.from_zeros(1000)
        m[56] = 1

        encoded = m.encode(Codec.Zstd)

        assert len(encoded) < len(m.encode(Codec.Raw))
        assert Mutibs.decode(encoded) == m
        assert Tibs.decode(encoded) == m

    def test_dtype_spec_rejects_unrepresentable_bit_length(self):
        with pytest.raises(ValueError):
            Dtype("u999999999999999999999999")

    def test_from_bin_reports_the_invalid_character(self):
        with pytest.raises(ValueError):
            Tibs.from_bin("0b10X01")
        with pytest.raises(ValueError):
