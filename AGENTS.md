...
- The package targets Python 3.11+ through `pyo3/abi3-py311`, so one abi3 wheel covers every supported version. The 3.11 floor is set by the buffer protocol, which only entered the limited ABI in that version. Continue supporting Python 3.11 unless explicitly instructed otherwise.
...
- Run `cargo fmt --all` before finishing Rust changes. Nothing in CI checks formatting: `.github/workflows/` holds only `new_ci.yaml`, which is autogenerated by maturin and should not be hand-edited, and `performance.yaml`. Formatting and clippy are therefore conventions, not gates, so run them locally.
- To rebuild the Python extension locally, use `make build` or `.venv/bin/maturin develop --release`.
- For the correctness loop, `make build-fast` uses the `fastdev` profile: the same optimisation level without the link-time work, which halves an incremental rebuild (about 25 seconds against 49 here). Every test passes on it.
- **Never take timings from a `fastdev` build.** Fat LTO with a single codegen unit is what decides how the hot kernels inline, and several of them are written the way they are because of it - see the comment on `count_pair_bits_with_wide` in `src/helpers/bitwise.rs`, where a refactor that removed duplication cost `count_or` and `count_xor` a factor of three under LTO and nothing without it. Numbers from `fastdev` look plausible and mean nothing. Benchmarks, `tests/performance_guards.py`, and releases all use `release`.
