...
This package contains an editable vector rebuild of the supplied APNG, simplified into a one-way transition from three boxes to `tibs`.
...
The plugin creates editable vector frames on a new Figma page. The frames use one shared ease-in-out sine timing curve sampled into 28 frames by default. The APNG and browser preview hold on the first frame for `initial_delay_ms` before the transition starts.
...
- `tibs-transition-preview.apng`: non-editable rendered preview of the generated animation.
"""
...
def write_outputs(config: dict) -> None:
    ...
    frames[0].save(
        OUT / "tibs-transition-preview.apng",
        save_all=True,
